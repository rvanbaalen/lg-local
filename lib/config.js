...
class ConfigManager {
    ...
    updateServiceConfig(serviceName, config) {
        ...
        this.config.services[serviceName] = { ...this.config.services[serviceName], ...config };
        return this.saveConfig();
    }

    // Check if service should run locally
    isServiceLocal(serviceName) {
        const serviceConfig = this.getServiceConfig(serviceName);
        return serviceConfig.mode === 'local';
    }

    // Check if service should auto-start
    shouldAutoStartService(serviceName) {
        const serviceConfig = this.getServiceConfig(serviceName);
        return serviceConfig.autoStart === true;
    }

    // Get service endpoint for remote connections
    getServiceEndpoint(serviceName) {
        const serviceConfig = this.getServiceConfig(serviceName);
        return {
            host: serviceConfig.host || 'localhost',
            port: serviceConfig.port,
            mode: serviceConfig.mode || 'local'
        };
    }
}
