...
const { CloudProtocolParser } = require('./protocol');
const { TLVParser } = require('./tlv');
...
class LGCloudServer {
    ...
    startAPIServer() {
        return new Promise((resolve, reject) => {
            this.apiServer = this.app.listen(this.config.apiPort, (error) => {
                if (error) {
                    reject(error);
                } else {
